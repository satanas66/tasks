package mongo.webVisibilityAnalytics;
...
public class WebVisibilityAnalyticsProjection {
    ...
    /**
     * Método encargado de obtener un cursor cuya proyección será la obtención de los KPIs que dependera del código de cliente ficticio (precuentaCode) pasado por parámetro
     * Se devolverá la última proyección actualizada según el campo timestamp
     *
     * @param precuentaCode
     * @return
     */
    public DBCursor findValuesKPIsFromWebVisibilityAnalytics(String precuentaCode) {
        try {
            LOG.info(WebVisibilityAnalyticsMessage.SEARCH_CLIENT + precuentaCode);
            instanceProjection();

            query = new BasicDBObject();
            query.put("precuentaCode", precuentaCode);
    ...
    /**
     * Método que genera un Array de objeto con todos los valores para los KPIs que pertenezcan a un código de cliente ficticio
     *
     * @param clientCode
     * @return
     */
    public Object[] getProjectionFromWebVisibilityAnalytics(String clientCode, BasicDBList kpiseg400, BasicDBList kpiseg401) {
        Object[] projection = null;
        BasicDBObject basicDBObject = getLastProjection(clientCode);
        if (basicDBObject != null) {
            projection = new Object[73];
            ...
            if (basicDBObject.get("kpiseg") != null) {
                ...
                if (kpiseg.get("KPISEG406") != null) {
                    projection[72] = kpiseg.get("KPISEG406");
                }
            }
