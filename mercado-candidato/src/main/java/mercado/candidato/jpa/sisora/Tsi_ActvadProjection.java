...
public class Tsi_ActvadProjection {
    ...
    /**
     * Método que buscqa el campo ct_merclie dado el código de actividad
     * @param co_actvad
     * @return campo ct_merclie
     */
    public String findCtMerClieByCoActvad(Integer co_actvad){
        String result = null;
        try{
            CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
            CriteriaQuery<String> criteriaQuery = criteriaBuilder.createQuery(String.class);
            Root<Tsi_Actvad> root = criteriaQuery.from(Tsi_Actvad.class);
            criteriaQuery.select(root.get(Tsi_Actvad_.ct_merclie));
            criteriaQuery.where(
                    criteriaBuilder.equal(root.get(Tsi_Actvad_.co_actvad), co_actvad)
            );
            result = entityManager.createQuery(criteriaQuery).getSingleResult();
