...
public class Execute3 {
    ...
    private static Logger LOG = Logger.getLogger(Execute3.class);
    ...
    private static void execution(MercadoPotencial mercadoPotencial) {
        if (mercadoPotencial.isContinuaProceso()) {
            List<List<Integer>> clientCodes = mercadoPotencial.getClientCodes("clientCodes3.txt");
    ...
    private static void setUp(MercadoPotencial mercadoPotencial, List<List<Integer>> clientCodes) {
        ...
        for (int i = 0; i < clientCodes.size(); i++) {
            MercadoPotencialThread hilo = new MercadoPotencialThread(" Hilo3");
            hilo.instancePatameters("C:/tasks/mercado-potencial-mongo/src/main/resources/", "MERCADO_POTENCIAL_3.xlsx");
