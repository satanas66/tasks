...
public class Execute5 {
    ...
    private static Logger LOG = Logger.getLogger(Execute5.class);
    ...
    private static void execution(MercadoPotencial mercadoPotencial) {
        if (mercadoPotencial.isContinuaProceso()) {
            List<List<Integer>> clientCodes = mercadoPotencial.getClientCodes("clientCodes5.txt");
    ...
    private static void setUp(MercadoPotencial mercadoPotencial, List<List<Integer>> clientCodes) {
        ...
        for (int i = 0; i < clientCodes.size(); i++) {
            MercadoPotencialThread hilo = new MercadoPotencialThread(" Hilo5");
            hilo.instancePatameters("C:/tasks/mercado-potencial-mongo/src/main/resources/", "MERCADO_POTENCIAL_5.xlsx");
